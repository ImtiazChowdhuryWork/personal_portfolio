...
## 0. Where We Left Off · 2026-05-11

> Live snapshot at the end of the most recent session — read this first when resuming work.

**Current version:** `v3.66.0` · DB schema is at 10 tables — no new columns this session. All work was on the public portfolio + dashboard UI.

### Most recent session (2026-05-11) — Project CRUD modal + public Project Details + iOS/Android mockup pair

**The last placeholder Add button is now wired up.** + Add Project on the dashboard opens the heaviest modal yet — name, status, Featured toggle, short + long descriptions, Tech Stack chip input, Features bulleted editor, Thumbnail dropzone, **Screenshots multi-image dropzone with drag-reorder**, App Store + Play Store + GitHub URLs, optional slug override. Edit reuses the same modal. Delete still works inline on the table row. All three CRUD modals (Experience, Skills, Projects) now follow the same pattern; the dashboard has no placeholder Add buttons left.

**New patterns introduced for this modal that will be reusable later:**
- **Multi-image dropzone with drag-reorder** (`.proj-shots-grid`) — drop multiple files at once, parallel uploads to `/uploads/screenshots/`, each upload becomes a 9:16 thumbnail card with hover-revealed × remove + ☰ drag handle, position pill (`#1`, `#2`, …) showing the public phone-mockup cycle order.
- **Section divider headers inside a modal** (`.proj-section-title`) — small uppercase Phosphor-icon labels with a dashed underline that group the heaviest modal yet into Basic / Descriptions / Tech Stack / Features / Images / Store & Repo Links / Advanced.

**Backend GORM zero-value bug fixed on `Project.Update`.** Same gotcha that's still open on `Experience.IsCurrent`: `db.Updates(struct)` skips zero values, so clearing a description or toggling Featured off wouldn't persist. `project_service.go` now uses `db.Model(&p).Select("name","slug","description",…).Updates(updates)` with an explicit allowlist so empty strings + `false` bools get written. (`IsCurrent` still needs the same fix — see §15.)

**Dashboard Projects table got the same treatment as Skills/Experience.** New 48×48 thumbnail column on the left with a dashed-border placeholder for projects with no thumbnail; new Tech preview column showing `{first_tech} +{count}`; Edit button alongside Delete.

**Public portfolio: Project Details modal — Apps Showcase finally has a "more info" surface.** New `components/ProjectDetail.js` + `components/project-detail.css`. Each AppShowcase slide gets a "View Details" button anchored to the **bottom-right corner** (translucent blur, primary border, lifts on hover). Click it → wide modal slides in over the page with: header (thumbnail + name + status badge + Featured star + tagline), screenshot gallery (horizontal scroll with snap-to-start, 9:16 phone-aspect cards), About paragraph (`long_description` finally rendered publicly), Tech Stack chips, Key Features bulleted cards, store + GitHub link buttons. Closes via floating × / Esc / backdrop click. Body scroll locked while open. Sections silently hide when their data is empty so sparse projects don't show empty headings.

**Apps Showcase slide padding bumped from 48px → 80px horizontal.** The 48×48 Swiper next/prev arrows sit at `left/right: 10px`, so they extended out to 58px from each edge — the previous 48px slide padding meant the **left arrow visually overlapped the slide content** (title, description, tech tags). Bumping `.app-slide` horizontal padding to `var(--space-20)` (80px) leaves ~22px of breathing room on each side.

**iOS + Android phone mockup pair on every slide.** `PhoneMockup.js` got a `variant: 'ios' | 'android'` parameter (default ios for back-compat) plus `startIndex` and `showDots`. Android variant skips the pill notch and renders a centered punch-hole camera (with subtle primary-blue lens highlight), 32px corner radius (vs 44px on iOS), cooler shell tint, slightly different side-button positions. AppShowcase per-slide loop now renders both phones into a `.phone-slot-pair` wrapper — iOS in front (z-index 2), Android behind and pulled left by 60px (z-index 1) so the pair reads as a "device family" promo shot. Both share the same screenshots array but cycle **out of sync** (Android starts one screenshot ahead) so visitors see two different screens at any moment. Float animation timings are offset by `-2s` on Android so the two phones breathe on opposite phases. Only iOS shows the dot indicator strip below so we don't render duplicates. Pair shrinks to 200px each on mobile (< 768px) with the overlap tightened to 50px.

**Public Tech Stack section now shows skill descriptions on hover.** Each skill row that has a non-empty `description` gets a floating tooltip card above it on `:hover` (and `:focus-within` for keyboard nav) — fades + slides up 6px, dark background with a double-caret arrow trick (outer matches the border, inner matches the bg, so the arrow blends seamlessly). The hovered row gets `z-index: 5` so the tooltip overlays neighbouring rows cleanly. The old `.skill-desc { display: none }` rule (a placeholder for hover JS that was never wired up) has been removed.

### Suggested next areas (in priority order)

1. **Fix `Experience.IsCurrent` GORM zero-value bug** — toggling Mark as Present *off* on an existing entry currently doesn't persist. Convert `IsCurrent bool` → `*bool` (mirrors the visibility flags pattern). Touches 4 spots: model, `cv_generator_service.go`, `seed.go`, plus a `boolPtr` helper.
2. **CV Generate polish** — pick from the punch list in §15 → CV / Resume (section controls, preview-before-save, design pass, richer content, image embedding).
3. **Tier 2 status extras** — `vacation_return_date` field + "Back on {date}" banner messaging.
...
6. **Refactor sub-editor patterns** — the chip input, bulleted editor, dropzone, and multi-image grid are now duplicated across Experience + Skill + Project modals. Extract into reusable `Editor.chips(containerId)`, `Editor.bullets(containerId)`, `Editor.dropzone(...)` helpers. Cosmetic but would cut the dashboard JS by ~30%.

### Earlier — Yesterday (2026-05-10, latest) — Experience + Skills CRUD modals end-to-end

**Two of the three "Add" buttons that used to just toast a placeholder are now full Add/Edit/Delete flows.** Only Projects is still a placeholder. This was the #1 priority gap from the prior session's "Suggested next areas".

**Experience modal — built from scratch.** Single modal handles both create (no id) and edit (id supplied), backed by the existing `POST/PUT/DELETE /api/v1/experience` endpoints. Headline UX choices:

- **Dates** use `flatpickr` (4.6.13) with the `monthSelect` plugin — readonly inputs that pop a month-grid calendar; values land directly in the `"M Y"` storage format ("Jan 2022") so no parser/formatter helpers are needed. Inline **"Mark as Present"** pill button next to End Date — toggling it clears + disables the End Date input and is the source of truth for `is_current` on submit.
- **Company logo** is a drag-and-drop dropzone (PNG/SVG/jpeg, 256×256 recommended, max 10 MB). On upload it swaps to a card showing the thumbnail + filename (timestamp prefix stripped) + natural dimensions + file size + Replace/Remove. Upload uses the existing `POST /api/v1/upload` with `folder=logos`. Public timeline now renders the logo as a 64×64 (48×48 mobile) thumbnail in each card's header.
- **Achievements** is a bulleted editor — each row is its own draggable card with a primary-blue bullet, an inline borderless input, and a remove ×. Press **Enter** in any row to insert a new empty row immediately below; **drag the ☰ handle** to reorder; the section never collapses to nothing (removing the last row clears + refocuses).
- **Tech Used** is a chip input — type a tech + Enter/comma → instantly converts to a primary-blue rounded chip with a small pop animation. Backspace on empty input deletes the last chip; pasting `"Flutter, Dart, Firebase"` fans out into one chip per item; blur commits any half-typed text so submits don't silently drop in-progress text. Case-insensitive de-dupe.
- **Per-section visibility toggles** — every non-required section (Logo, Type, Location, Description, Achievements, Tech Used) has a small "Visible / Hidden" pill toggle in its label header. Off state dims the section content (45% opacity + light grayscale) while keeping inputs editable, and flips the pill red. Backed by 6 new `*bool` columns on the `experiences` table (`*bool` not `bool` so GORM's `Updates` actually persists a `false` toggle — plain bool's zero value silently gets skipped). Public timeline checks each `show_*` flag before rendering the corresponding piece. `null`/`undefined` (legacy rows) is treated as visible, matching the DB column default.

**Skill modal — built from scratch.** Same Add+Edit+Delete pattern. Fields: name, **category** (dropdown with the 5 predefined keys mapped to friendly labels — `core` → "Core (framework / language)", `state_management` → "State Management", etc. — plus a "+ Custom category…" escape hatch that reveals a text input), **proficiency** (range slider linked two-way to a number input + `%` suffix; defaults to 80; type or drag, both stay in sync), description, **icon** (same dropzone + card pattern as Experience logo, smaller, `folder=icons`), sort order. Validation matches the backend: name + category required, percentage 0–100. Reuses `formatExpFileSize` from the Experience modal.

**Dashboard Skills list redesigned as portfolio preview.** The plain Name/Category/%/Actions table is gone. Skills are now grouped by category in the canonical order the public Tech Stack uses (Core → State Management → Backend & APIs → Payments → Tools & DevOps), each row is a real `SkillBar`-styled element (icon or 2-letter initial fallback + name + blue gradient bar at its real percentage + percentage value), with Edit/Delete buttons inline on the right. Animation skipped (admins don't need the fill-from-zero theatrics; bars render at full width immediately). Custom-category skills get bucketed into a separate **"Other"** group at the bottom with a red `Won't show publicly` badge — surfaces silently-dropped data so the user notices and re-categorises.

**Dashboard tables also got Logo / Icon thumbnails + Edit buttons.** Experience table now has a 36×36 logo column on the left (with a dashed-border placeholder for entries with no logo); Skills entries pre-redesign also got a 32×32 icon column before being absorbed into the new grouped view.

**Cache-busting `@import` chain bug fixed.** `index.html` had `?v=N` on the parent `main.css` but `main.css`'s own `@import './components/timeline.css'` lines didn't — so bumping the parent didn't propagate. Browser kept serving stale `timeline.css`. Fixed by adding `?v=N` to the `@import` URL too whenever component CSS changes. Bumped a few times this session as visual changes shipped.

**Tech Stack categorisation tangent (built then dropped).** Earlier in the session I built a "categorised tech-stack breakdown" feature on Experience entries (`TechLayers []TechLayer` JSON type, layered card editor in the modal, icon-led grouped sections on the public timeline). User saw it and said "doesn't look good — drop it". Fully reverted on every layer (Go model + service, modal HTML/CSS/JS, public timeline render + CSS, and the `tech_stack` Postgres column was dropped via direct `ALTER TABLE`). The chip input is back where it was. Mentioned here as a heads-up so the column-drop doesn't read like an unrelated migration in `git log`.

### Known related bug deliberately not fixed this session

`Experience.IsCurrent` is plain `bool` (not `*bool`). GORM's `Updates(struct)` skips zero-value fields, so toggling **Mark as Present** *off* on an existing entry won't persist (the new value `false` is `bool`'s zero). The 6 new visibility flags use `*bool` and don't have this issue. Fixing `IsCurrent` requires touching 4 spots (model, `cv_generator_service.go`, `seed.go`, plus a `boolPtr` helper) so it's noted in §15 instead of being silently bundled in.

### Earlier in the day (2026-05-10, late) — async reply queue + optimistic UI
...
*(Yesterday's "Suggested next areas" list was reconciled into today's at the top of §0 — Project modal + Project detail modal both shipped today.)*
...
| is_current | BOOLEAN | Marks active job. ⚠ Plain `bool` — toggling off via partial PATCH currently doesn't persist (GORM `Updates` skips zero values). See §15 |
...
| show_logo | BOOLEAN | Default `true`. Per-section visibility — when false, the public timeline skips the logo thumbnail. Stored as `*bool` on the model so a `false` toggle persists through GORM `Updates`; legacy `null` rows are treated as visible by the public renderer |
| show_type | BOOLEAN | Default `true`. Hides the small employment-type badge in the card header when false |
| show_location | BOOLEAN | Default `true`. Hides the "· {location}" suffix after the company name when false |
| show_description | BOOLEAN | Default `true`. Hides the description paragraph when false |
| show_achievements | BOOLEAN | Default `true`. Hides the achievements bullet list when false |
| show_tech_used | BOOLEAN | Default `true`. Hides the row of tech tags at the bottom of the card when false |
...
5. **Apps Showcase** — Swiper slider. Each slide: app name, description, tech tags, features, App Store/Play Store badges, and a **"View Details"** pill anchored to the bottom-right corner. **Two phone mockups per slide** — iOS in front (pill notch + dynamic-island accent dot) and Android behind, pulled left by 60px so they overlap as a "device family" promo shot. Both phones cycle the same screenshots **out of sync** (Android starts one screenshot ahead) and float on opposite phases (`-2s` animation delay). Click **View Details** → wide modal slides in showing the project's `long_description`, full screenshot gallery (horizontal scroll), all tech, all features, and store/repo buttons. Closes via floating × / Esc / backdrop click.
...
- **📱 Projects** — Table of all apps with a 48×48 thumbnail column, a tech preview column showing `{first_tech} +{count}`, and Edit + Delete buttons inline. **+ Add Project** opens the heaviest modal in the dashboard: Basic (name / status / Featured toggle pill / sort order), Descriptions (short + long), Tech Stack (chip input), Features (bulleted editor), Images (Thumbnail dropzone + **Screenshots multi-image dropzone with parallel uploads, drag-handle reorder, position pill on each tile**), Store & Repo Links (App Store / Play Store / GitHub URLs), Advanced (optional slug override). Visually grouped with `.proj-section-title` Phosphor-icon dividers
- **⚡ Skills** — Portfolio-preview style: skills grouped by category in the canonical public-site order (Core / State Management / Backend & APIs / Payments / Tools & DevOps), each row rendered as a real `SkillBar`-styled element (icon or 2-letter initial fallback, name, blue gradient bar at the real percentage, percentage value) with inline Edit + Delete buttons. Custom-category entries fall into a separate "Other" group with a red `Won't show publicly` warning badge. **+ Add Skill** opens a modal with name, category dropdown (5 predefined keys + Custom escape hatch), proficiency (range slider linked to a number input), description, icon dropzone (PNG/SVG, square, 128×128 recommended, max 10 MB), sort order
- **💼 Experience** — Table of work history with logo thumbnails on the left, Edit + Delete actions inline. **+ Add Experience** opens a modal with: company / role / start date (flatpickr month picker) / end date + inline "Mark as Present" toggle / type select / location / company logo dropzone / description textarea / **bulleted achievements editor** (each bullet is its own draggable row, Enter inserts a new row, drag-handle reorders) / **Tech Used chip input** (Enter or comma → chip, Backspace deletes last, paste-split on commas) / sort order. Every non-required section has a **"Visible / Hidden" toggle pill** in its label header so individual sections can be suppressed on the public timeline without deleting the data — backed by 6 new `show_*` columns on the `experiences` table
...
- [x] **6 per-section visibility flags on `Experience`** — `ShowLogo`, `ShowType`, `ShowLocation`, `ShowDescription`, `ShowAchievements`, `ShowTechUsed`. Each is `*bool` (not `bool`) with `gorm:"default:true"` so a `false` toggle persists through `Updates` (plain `bool`'s zero value would be silently skipped). DB default backfills existing rows with `true` on AutoMigrate, so legacy entries stay fully visible
- [x] **GORM zero-value bug fix on `Project.Update`** — `db.Updates(struct)` skips zero values (empty strings, false bools), so clearing a description or toggling Featured off wouldn't persist. `project_service.go` now uses `db.Model(&p).Select("name","slug","description",…).Updates(updates)` with an explicit allowlist of editable columns. ID / CreatedAt / DeletedAt / UpdatedAt are deliberately excluded so the dashboard can't accidentally reset them. Same gotcha is still open on `Experience.IsCurrent` — see §15
...
- [x] **Experience CRUD modal** — full Add/Edit form replacing the placeholder Toast. flatpickr month picker for start/end dates, inline "Mark as Present" toggle, drag-and-drop logo dropzone (uploads to `/uploads/logos/...` via the existing `/upload` endpoint, swaps to a card with thumbnail + filename + dimensions + size + Replace/Remove), bulleted achievements editor (Enter adds a new row, drag handle reorders, never collapses to nothing), Tech Used chip input (Enter/comma adds, Backspace deletes last, paste-split, blur commits), sort order. Edit button + 36×36 logo thumbnail column added to the table
- [x] **Per-section visibility toggles on the Experience modal** — every non-required section (Logo / Type / Location / Description / Achievements / Tech Used) has a small "Visible / Hidden" pill toggle in its label header. Off state dims the section content (45% opacity + light grayscale) while keeping inputs editable; flips the pill red. Public timeline checks each `show_*` flag before rendering the corresponding piece. New helpers: `toggleExpVisibility`, `setExpVisibility`, `getExpVisibility`, `resetExpVisibility`, `applyExpVisibilityFrom`. `null`/`undefined` is treated as visible for backward-compat with legacy rows
- [x] **Skill CRUD modal** — full Add/Edit form replacing the placeholder Toast. Fields: name, category dropdown (5 predefined keys with friendly labels, `+ Custom category…` escape hatch that reveals a text input — also auto-detects custom values when editing), **proficiency** (range slider two-way bound to a number input + `%` suffix; defaults to 80), description, icon dropzone (`folder=icons`, 128×128 recommended), sort order. Validation matches the backend
- [x] **Dashboard Skills list redesigned as portfolio preview** — replaces the plain Name/Category/%/Actions table. Skills grouped by category in the canonical public-site order (Core / State Management / Backend & APIs / Payments / Tools & DevOps), each row rendered as a real `SkillBar`-styled element (icon or 2-letter initial fallback + name + blue gradient bar at real percentage + percentage value) with inline Edit/Delete. Animation skipped (admins don't need fill-from-zero theatrics). Custom-category entries get bucketed into an "Other" group at the bottom with a red **Won't show publicly** badge so silently-dropped data is surfaced. Reuses `.skill-icon` / `.skill-bar-track` / `.skill-bar-fill` from `skill-bar.css` so the visual matches the public site exactly
- [x] Public Experience timeline renders `company_logo` as a 64×64 (48×48 mobile) rounded thumbnail at the start of each card's header. `object-fit: contain` with 4px padding so non-square brand marks aren't cropped. `onerror` silently hides broken paths
- [x] `@import` cache-bust chain fix in `main.css` — bumping `?v=N` on the parent `<link>` doesn't propagate to `@import` URLs, so component CSS stayed cached. Now `@import './components/timeline.css?v=N'` is bumped together with the parent stylesheet whenever a component changes
- [x] **Project CRUD modal** — full Add/Edit form replacing the placeholder Toast on `+ Add Project`. Heaviest modal yet, visually grouped with section dividers (Basic / Descriptions / Tech Stack / Features / Images / Store & Repo Links / Advanced). Featured is a Mark-as-Present-style toggle pill. Tech Stack reuses the chip input pattern; Features reuses the bulleted editor pattern. Thumbnail uses the single-image dropzone; **Screenshots is a NEW multi-image dropzone with drag-reorder** — parallel uploads to `/uploads/screenshots/`, each upload becomes a 9:16 thumbnail card with hover-revealed × + ☰ drag handle + `#N` position pill (controls the public phone-mockup cycle order). Slug field sits under Advanced — leave blank and the backend auto-generates from the name
- [x] Dashboard Projects table redesigned — 48×48 thumbnail column on the left (with dashed-border placeholder for missing thumbs), Tech preview column (`Flutter +3`), Edit button alongside Delete
- [x] **Public Project Details modal** — new `components/ProjectDetail.js` + `components/project-detail.css`. Each AppShowcase slide gets a "View Details" pill anchored to the bottom-right corner of the slide (translucent dark background with backdrop blur, primary border, lifts on hover). Click → wide modal (`max-width: 960px`) slides in showing: header (thumbnail + name + status badge + ⭐ Featured + tagline), screenshot gallery (horizontal scroll with snap-to-start, 9:16 phone-aspect cards, custom scrollbar), About paragraph (`long_description` finally rendered publicly), Tech Stack chips, Key Features bulleted cards, store + GitHub link buttons. Closes via floating × / Esc / backdrop click. Body scroll locked while open. Empty sections silently dropped
- [x] Apps Showcase slide horizontal padding bumped from 48px → 80px so the 48×48 Swiper next/prev arrows (at `left/right: 10px`) don't overlap the slide content. ~22px breathing room each side
- [x] **iOS + Android phone mockup pair on every slide.** `PhoneMockup.js` got `variant: 'ios' | 'android'` (default ios for back-compat) plus `startIndex` and `showDots` params. Android variant skips the pill notch and renders a centered punch-hole camera (with subtle primary-blue lens highlight), 32px corner radius vs 44px on iOS, cooler shell tint, slightly different side-button positions. AppShowcase per-slide loop renders both phones into a `.phone-slot-pair` wrapper — iOS in front (z-index 2), Android behind and pulled left by 60px (z-index 1). Both share the same screenshots array but cycle out of sync (Android starts at index 1) so visitors see two different screens at any moment. Float animation timings offset by `-2s` on Android. Only iOS shows the dot indicator strip below
- [x] **Public Tech Stack tooltip on hover** — each skill row that has a non-empty `description` gets a floating tooltip card above it on `:hover` / `:focus-within`. Fades + slides up 6px. Dark bg with a double-caret arrow (outer matches border colour, inner matches bg, so the arrow blends seamlessly). The hovered row gets `z-index: 5` so the tooltip overlays neighbours cleanly. Skill descriptions were stored in the DB but never rendered publicly until now (the old `.skill-desc { display: none }` placeholder rule has been removed)
...
- [x] `experiences` extended with 6 `BOOLEAN DEFAULT TRUE` columns (`show_logo`, `show_type`, `show_location`, `show_description`, `show_achievements`, `show_tech_used`) for per-section visibility — Postgres backfills existing rows with `true` so all legacy entries stay fully visible
...
### Dashboard
- [x] ~~**Add Project modal**~~ — done. Heaviest modal in the dashboard: name, status, Featured toggle, short + long descriptions, Tech Stack chips, Features bulleted editor, Thumbnail dropzone, **multi-image Screenshots dropzone with drag-reorder + position pills**, App Store / Play Store / GitHub URLs, optional slug override.
- [x] ~~**Add Skill modal**~~ — done. Range slider + linked number for percentage, category dropdown with custom escape hatch, drag-and-drop icon upload.
- [x] ~~**Add Experience modal**~~ — done. flatpickr month pickers, "Mark as Present" toggle, drag-and-drop logo upload, bulleted achievements editor, Tech Used chip input, per-section visibility toggles.
- [x] ~~**Edit functionality**~~ — done for all three (Skills, Experience, Projects each use a single modal that handles both Add and Edit).
- [x] ~~**Image upload UI**~~ — done for company logos, skill icons, project thumbnails, **and project screenshots (multi-image grid with reorder)**. All reuse the same dropzone pattern — drag/click → upload via `/upload` → swap to a preview card.
...
- [ ] **`Experience.IsCurrent` GORM zero-value bug** — `IsCurrent` is plain `bool` (not `*bool`). GORM's `Updates(struct)` skips zero-value fields, so toggling **Mark as Present** *off* on an existing entry doesn't persist (the new value `false` is `bool`'s zero). Fix: convert to `*bool` mirroring the new `Show*` flags pattern. Touches 4 spots: model, `cv_generator_service.go` (currently reads `e.IsCurrent` as bool), `seed.go` (3 literal-`bool` initialisers), plus a `boolPtr(true/false)` helper. The new visibility flags don't have this issue because they were built `*bool` from day one.
...
- ~~Dashboard modal for creating/editing **skills**~~ ✅ Done (range slider, category dropdown with custom escape hatch, icon dropzone)
- ~~Dashboard modal for creating/editing **experience**~~ ✅ Done (flatpickr dates, Mark as Present, drag-and-drop logo, bulleted achievements editor, Tech Used chips, per-section visibility toggles)
- ~~Dashboard modal for creating/editing **projects**~~ ✅ Done (Featured toggle, Tech Stack chips, Features bullets, single-image Thumbnail dropzone, **multi-image Screenshots dropzone with drag-reorder**, store/repo URLs, slug override)
...
- ~~Project detail modal (clicking "View Details" on app slide)~~ ✅ Done (`ProjectDetail.js` + `project-detail.css`; opens a wide modal with header, screenshot gallery, About paragraph from `long_description`, tech chips, features, store + GitHub buttons; Esc / × / backdrop close)
...
| 3.57.0 | 2026-05-10 | **Experience CRUD modal — built from scratch.** Replaces the placeholder Toast on `+ Add Experience`. Single modal handles both create (no id) and edit (id supplied), backed by the existing `POST/PUT/DELETE /experience` endpoints. Headline UX: `flatpickr` 4.6.13 with the `monthSelect` plugin for start/end dates (readonly inputs, month-grid calendar popup, values land directly in the `"M Y"` storage format so no parser/formatter helpers needed); inline "Mark as Present" pill toggle next to End Date — toggling it clears + disables the End Date input and is the source of truth for `is_current` on submit; drag-and-drop **company logo dropzone** (PNG/SVG/jpeg, 256×256 recommended, max 10 MB) that swaps to a card with thumbnail + filename (timestamp prefix stripped) + natural dimensions + file size + Replace/Remove (uploads to `/uploads/logos/...` via the existing `/upload` endpoint with `folder=logos`); **bulleted achievements editor** — each bullet is its own draggable row with a primary-blue ▸, an inline borderless input, and a remove ×, Enter inserts a new empty row immediately below, drag the ☰ handle to reorder, section never collapses (removing the last row clears + refocuses); **Tech Used chip input** — type a tech + Enter or comma → instantly converts to a primary-blue rounded chip with a small pop animation, Backspace on empty input deletes the last chip, paste-split on commas/newlines, blur commits any half-typed text, case-insensitive de-dupe; sort order. Edit button + 36×36 logo thumbnail column added to the dashboard table | dashboard.html, experience.js, timeline.css |
| 3.57.1 | 2026-05-10 | **Per-section visibility toggles on the Experience modal.** Every non-required section (Logo / Type / Location / Description / Achievements / Tech Used) has a small "Visible / Hidden" pill toggle in its label header. Off state dims the section content (45% opacity + light grayscale) while keeping inputs editable, and flips the pill red. Backed by 6 new `*bool` fields on the `Experience` model (`ShowLogo`, `ShowType`, `ShowLocation`, `ShowDescription`, `ShowAchievements`, `ShowTechUsed`), all `gorm:"default:true"`. Used `*bool` (not `bool`) because GORM's `Updates(struct)` skips zero-value fields — plain `bool`'s zero is `false`, so a toggle-off would silently get dropped. Public timeline checks each `show_*` flag before rendering the corresponding piece; `null`/`undefined` is treated as visible for legacy rows (matching the DB column default). Also flagged the same GORM zero-value gotcha on `Experience.IsCurrent` as a deliberate-not-fix-this-session known issue (touches 4 spots, separate work). New JS helpers: `toggleExpVisibility`, `setExpVisibility`, `getExpVisibility`, `resetExpVisibility`, `applyExpVisibilityFrom` | experience.go, dashboard.html, experience.js |
| 3.58.0 | 2026-05-10 | **Skill CRUD modal — built from scratch.** Replaces the placeholder Toast on `+ Add Skill`. Same Add+Edit+Delete pattern as the Experience modal. Fields: name, **category** (dropdown with the 5 predefined keys mapped to friendly labels — `core` → "Core (framework / language)", `state_management` → "State Management", etc. — plus a "+ Custom category…" escape hatch that reveals a text input; auto-detects custom values when editing), **proficiency** (range slider linked two-way to a number input + `%` suffix; defaults to 80; type or drag, both stay in sync), description, **icon** (same dropzone + card pattern as the Experience logo, smaller, `folder=icons`), sort order. Validation matches the backend (name + category required, percentage 0–100). Reuses `formatExpFileSize` from the Experience modal | dashboard.html |
| 3.58.1 | 2026-05-10 | **Dashboard Skills list redesigned as portfolio preview.** Replaces the plain Name/Category/%/Actions table. Skills grouped by category in the canonical public-site order (Core / State Management / Backend & APIs / Payments / Tools & DevOps), each row rendered as a real `SkillBar`-styled element (icon or 2-letter initial fallback + name + blue gradient bar at real percentage + percentage value) with inline Edit/Delete buttons. Animation skipped (admins don't need fill-from-zero theatrics; bars render at full width immediately). Custom-category entries get bucketed into a separate "Other" group at the bottom with a red **Won't show publicly** warning badge — surfaces silently-dropped data so the user notices and re-categorises. Reuses `.skill-icon` / `.skill-bar-track` / `.skill-bar-fill` from `skill-bar.css` so the visual matches the public site exactly | dashboard.html |
| 3.59.0 | 2026-05-10 | **`@import` cache-bust chain fix in `main.css`.** `index.html` had `?v=N` on the parent `<link rel="stylesheet" href="/assets/css/main.css?v=N">` but `main.css`'s own `@import './components/timeline.css'` line didn't. So when component CSS changed, browsers refreshed `main.css` (because the parent URL changed) but kept the cached `timeline.css` (because the imported URL was identical). Fixed by adding `?v=N` to the `@import` URL too whenever component CSS changes. Bumped a few times this session as visual changes shipped (`timeline.css` and `experience.js` were repeatedly affected) | main.css, index.html |
| 3.59.1 | 2026-05-10 | **Tech Stack categorisation tangent — built then dropped on user feedback.** Added a "categorised tech-stack breakdown" feature on Experience entries: new `TechLayers []TechLayer` JSON type (`models/tech_layers.go`), `TechStack` field on Experience model with auto-flatten-to-`TechUsed` in the service layer, layered card editor in the modal with per-layer chip input + "+ Add layer" dropdown, icon-led grouped-section render on the public timeline. User saw the public timeline and said "doesn't look good — drop the idea". Fully reverted on every layer (model field removed, service flatten removed, `tech_layers.go` deleted, modal HTML/CSS/JS reverted to the chip input that preceded this work, public timeline render reverted to flat tags + CSS removed). Postgres column was dropped via direct `ALTER TABLE experiences DROP COLUMN tech_stack` after confirming nothing read or wrote to it. Mentioned in the changelog so the column-drop migration in `git log` doesn't read like an unrelated change | experience.go, experience_service.go, dashboard.html, experience.js, timeline.css, Postgres `experiences.tech_stack` (dropped) |
| 3.60.0 | 2026-05-10 | README brought current — § 0 bumped to v3.60.0 with full session summary covering the Skills + Experience CRUD modals, the per-section visibility flags, and the dropped Tech Stack tangent; § 0 Suggested next areas reordered (Project modal now top of list; new IsCurrent fix at #2); § 10 `experiences` schema gains the 6 new `show_*` columns + a warning on `is_current`; § 11 dashboard tabs descriptions for Skills + Experience rewritten to match the new modals + preview list; § 14 backend / frontend / database lists extended with all the session's additions; § 15 Known Issues marks Skills + Experience CRUD/Edit/Image-upload as done and adds the `IsCurrent` GORM zero-value bug as a new entry; § 16 Roadmap Phase 2 reconciled (Skills + Experience modals ✅, Projects still pending) | README.md |
| 3.61.0 | 2026-05-11 | **Skill descriptions now visible on the public Tech Stack section.** Each skill row that has a non-empty `description` gets a floating tooltip card above it on `:hover` and `:focus-within`. Tooltip sits on a dark background, fades + slides up 6px on appear, with a double-caret arrow trick (outer caret matches the border colour, inner caret matches the bg, so the arrow blends seamlessly). The hovered row gets `z-index: 5` so the tooltip overlays neighbouring rows cleanly. Skill descriptions were stored in the DB but never rendered publicly until now — the old `.skill-desc { display: none }` placeholder rule (a leftover from "Shown on hover via JS" that was never wired up) has been removed | SkillBar.js, skill-bar.css |
| 3.62.0 | 2026-05-11 | **Project CRUD modal — built from scratch.** Last of the three placeholder Add buttons. Single modal handles both create + edit, backed by the existing `POST/PUT/DELETE /projects` endpoints. Heaviest modal in the dashboard, visually grouped with `.proj-section-title` Phosphor-icon dividers into Basic / Descriptions / Tech Stack / Features / Images / Store & Repo Links / Advanced. Featured is a Mark-as-Present-style toggle pill (inline label + small description). Tech Stack reuses the chip input pattern from Experience tech_used (`proj-tech-input` IDs, scoped duplicate of `addExpTechChip` etc.); Features reuses the bulleted editor pattern from Experience achievements (drag-handle reorder, Enter inserts a new row); Thumbnail uses the single-image dropzone pattern; Screenshots is a NEW multi-image dropzone — accepts multiple files at once, parallel uploads to `/uploads/screenshots/` via `Promise.all`, each upload becomes a 9:16 thumbnail card in `.proj-shots-grid` with hover-revealed × remove + ☰ drag handle + `#N` position pill (the public phone mockup cycles in this exact order). Slug field sits under Advanced — leave blank and the backend auto-generates from name. Status select with three options (🟢 Live / 🟡 In Development / ⚪ Archived). Validation: name required | dashboard.html |
| 3.62.1 | 2026-05-11 | **`Project.Update` GORM zero-value bug fix.** Same gotcha that's still open on `Experience.IsCurrent`: `db.Updates(struct)` skips zero values (empty strings, false bools), so clearing a description or toggling Featured off wouldn't persist. `project_service.go` now uses `db.Model(&p).Select("name","slug","description","long_description","tech_stack","features","screenshots","thumbnail","app_store_url","play_store_url","github_url","status","featured","sort_order").Updates(updates)` — explicit allowlist of editable columns. ID / CreatedAt / DeletedAt / UpdatedAt are deliberately excluded so the dashboard can't accidentally reset them by sending defaults | project_service.go |
| 3.62.2 | 2026-05-11 | Dashboard Projects table redesigned to match the Skills + Experience pattern — 48×48 thumbnail column on the left (with dashed-border placeholder + `ph-image` icon for missing thumbs); new Tech preview column showing `{first_tech} +{count}` (e.g. `Flutter +3`); Edit button alongside Delete | dashboard.html |
| 3.63.0 | 2026-05-11 | **Public Project Details modal** — Apps Showcase finally has a "more info" surface. New `components/ProjectDetail.js` + `components/project-detail.css`. Each AppShowcase slide gets a "View Details" pill button; click it → wide modal (`max-width: 960px`) slides in over the page with header (96×96 thumbnail + name + status badge `🟢 Live` / `🟡 In Development` / `⚪ Archived` + ⭐ Featured badge + `description` as tagline), screenshot gallery (horizontal scroll with snap-to-start, 9:16 phone-aspect cards, custom thin scrollbar, hover lift), About paragraph (`long_description` finally rendered publicly — newlines preserved as `<br>`, HTML escaped for safety), Tech Stack chips, Key Features bulleted cards (same `▸` primary-blue caret as the experience timeline), and Get It buttons (App Store / Google Play / GitHub). Closes via floating × / Esc / backdrop click. Body scroll locked while open. Scroll resets to top on every open. Empty sections silently dropped so sparse projects don't show empty headings. New modal element added to `index.html` body, new `<script>` tag for `ProjectDetail.js`, new `@import` for the CSS in `main.css` | ProjectDetail.js (new), project-detail.css (new), AppShowcase.js, main.css, index.html |
| 3.63.1 | 2026-05-11 | View Details button moved out of `.app-slide-content` and **anchored to the bottom-right corner of the slide** — `position: absolute; bottom: var(--space-8); right: var(--space-8)`, on a translucent dark backdrop with `backdrop-filter: blur(8px)`, primary border + soft shadow, lifts 2px on hover with a brighter shadow. Trailing arrow nudges right on hover. Mobile (< 640px) shrinks the button slightly and tightens the corner inset. `.app-slide` got `position: relative` to anchor it | AppShowcase.js, project-detail.css, app-slider.css |
| 3.64.0 | 2026-05-11 | Apps Showcase slide horizontal padding bumped from `var(--space-12)` (48px) → `var(--space-20)` (80px). The Swiper next/prev arrows are 48×48 circles at `left/right: 10px` (Swiper default), so they extended out to 58px from each edge — the previous 48px slide padding meant the **left arrow visually overlapped the slide content** (title, description, tech tags). The new 80px padding leaves ~22px of breathing room between each arrow and the start of the content | app-slider.css |
| 3.65.0 | 2026-05-11 | **iOS + Android phone mockup pair on every slide.** `PhoneMockup.js` got new params: `variant: 'ios' \| 'android'` (default ios for back-compat), `startIndex: number` (default 0), and `showDots: boolean` (default true). Android variant skips the pill notch and renders a centered punch-hole camera (`.phone-punch-hole` with subtle primary-blue lens highlight via `::after`); 32px corner radius vs 44px on iOS; cooler shell tint; slightly different side-button positions. AppShowcase per-slide loop now renders both phones into a `.phone-slot-pair` flex wrapper — iOS in front (z-index 2), Android behind and pulled left by 60px (`margin-left: -60px`, z-index 1) so the pair reads as a "device family" promo shot. Both phones share the same `screenshots` array but cycle **out of sync** — Android starts at `startIndex: 1` so visitors see two different screens at any moment. Float animation timings are offset by `-2s` on Android (`.phone-mockup--android { animation-delay: -2s }`) so the two phones breathe on opposite phases. Only iOS shows the dot indicator strip below (Android is rendered with `showDots: false`). Pair shrinks to 200px each on mobile (< 768px) with the overlap tightened to 50px | PhoneMockup.js, phone-mockup.css, AppShowcase.js |
| 3.66.0 | 2026-05-11 | README brought current — § 0 bumped to v3.66.0 with full session summary covering the Project CRUD modal, the public Project Details modal, the iOS/Android mockup pair, the slide-padding fix, the Skill description hover tooltip, and the View Details bottom-right placement; Suggested next areas reordered (Project modal + Project detail modal both ✅; `IsCurrent` fix now top of list, plus a new "refactor sub-editor patterns" item flagging the chip / bullets / dropzone duplication across the three CRUD modals); § 11 dashboard description for Projects rewritten to match the new modal + table; § 11 portfolio Apps Showcase description updated for the iOS/Android pair + View Details modal; § 14 backend (Project Update zero-value fix) + frontend (6 new entries) lists extended; § 15 Known Issues marks every Add modal + Edit functionality + image upload as done; § 16 Roadmap Phase 2 reconciled (all three modals ✅) and Project detail modal moved from pending to done | README.md |
