...
**Current version:** `v3.60.0` · DB schema is at 10 tables. `experiences` gains 6 new `BOOLEAN DEFAULT TRUE` per-section visibility columns (`show_logo`, `show_type`, `show_location`, `show_description`, `show_achievements`, `show_tech_used`) added on next startup via AutoMigrate. No new tables.

### Most recent session (2026-05-10, latest) — Experience + Skills CRUD modals end-to-end
...
1. **Project Add modal** — last of the three placeholder Add buttons; same Add/Edit/Delete pattern Skills + Experience now follow. The model is heaviest (screenshots array, store URLs, features array, slug uniqueness, status enum) so it's a slightly bigger lift than the other two.
2. **Fix `Experience.IsCurrent` GORM zero-value bug** — see "Known related bug" above. Convert to `*bool` (mirrors the visibility flags pattern) so toggling Present off on an existing entry actually persists.
3. **CV Generate polish** — pick from the punch list in §15 → CV / Resume (section controls, preview-before-save, design pass, richer content).
4. **Tier 2 status extras** — `vacation_return_date` field + "Back on {date}" banner messaging (we did Tier 1 of the Status overhaul; Tier 2 was deferred).
5. **Auto-acknowledgement email to visitors** — send a "thanks, I'll reply soon" the moment the contact form is submitted (currently only the dashboard admin gets the SMTP path).
6. **Testimonials section on the public portfolio** — table exists in DB, no UI.
7. **Project detail modal** — clicking "View Details" on an app slide.
...
- **📱 Projects** — Table of all apps with delete button (Add/Edit form still pending — the only placeholder remaining)
...
### Dashboard
- [ ] **Add Project modal** — last of the three placeholder Add buttons. Skills + Experience are now fully wired (Add/Edit/Delete with file upload). Project follows the same pattern but is the heaviest model (screenshots array, store URLs, features, slug uniqueness, status enum).
...
- [x] ~~**Edit functionality**~~ — done for Skills + Experience (single modal handles both Add and Edit). Projects still delete-only until the Add modal lands.
- [x] ~~**Image upload UI**~~ — done for company logos and skill icons. Both reuse the same dropzone UX (drag/click → uploads via `/upload` → swaps to a card with thumbnail + filename + dimensions + size + Replace/Remove). Project screenshots will reuse this when the Project modal lands.
...
- Dashboard modal for creating/editing **projects** — last remaining placeholder (heavier model: screenshots, store URLs, features, slug, status)
